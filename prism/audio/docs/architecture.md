...
**Projection Research In Speech Modeling**

## System Overview

PRISM enables text-to-speech synthesis by projecting frozen LLM hidden states to frozen TTS decoder features through a lightweight, trainable adapter. This modular approach allows combining any pretrained LLM with any pretrained TTS model.
...
- Parameters: 1B-70B+ (model-dependent)
...
### 3. TTS Decoder: Pretrained TTS Model

**Typical Specifications:**
- Parameters: 100M-5B+ (model-dependent)
...
**Frozen:**
- Base LLM encoder (100% frozen)
- TTS decoder (100% frozen)
...
LLM Encoder (frozen)
    ↓
Hidden States [B, L, H_llm]
    ↓
Audio Projector
    ↓
Projected Features [B, L, H_tts]
    ↓
TTS Decoder (frozen)
    ↓
Audio Output
