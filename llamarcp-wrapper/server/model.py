...
class LlamaProxy:
    def __init__(self, models: List[ModelSettings]) -> None:
        ...
        self._current_model: Optional[llamarcp.Llama] = None
    ...
    def __call__(self, model: Optional[str] = None) -> llamarcp.Llama:
        if model is None:
    ...
    @staticmethod
    def load_llama_from_model_settings(settings: ModelSettings) -> llamarcp.Llama:
        chat_handler = None
        if settings.chat_format == "llava-1-5":
            assert settings.clip_model_path is not None, "clip model not found"
            if settings.hf_model_repo_id is not None:
                chat_handler = (
                    llamarcp.llama_chat_format.Llava15ChatHandler.from_pretrained(
                ...
                chat_handler = llamarcp.llama_chat_format.Llava15ChatHandler(
            ...
            if settings.hf_model_repo_id is not None:
                chat_handler = (
                    llamarcp.llama_chat_format.ObsidianChatHandler.from_pretrained(
                        repo_id=settings.hf_model_repo_id,
                        filename=settings.clip_model_path,
                        verbose=settings.verbose,
                    )
                ...
                chat_handler = llamarcp.llama_chat_format.ObsidianChatHandler(
                    clip_model_path=settings.clip_model_path, verbose=settings.verbose
                )
            ...
            if settings.hf_model_repo_id is not None:
                chat_handler = (
                    llamarcp.llama_chat_format.Llava16ChatHandler.from_pretrained(
                        repo_id=settings.hf_model_repo_id,
                        filename=settings.clip_model_path,
                        verbose=settings.verbose,
                    )
                ...
                chat_handler = llamarcp.llama_chat_format.Llava16ChatHandler(
                    clip_model_path=settings.clip_model_path, verbose=settings.verbose
                )
            ...
            if settings.hf_model_repo_id is not None:
                chat_handler = (
                    llamarcp.llama_chat_format.MoondreamChatHandler.from_pretrained(
                        repo_id=settings.hf_model_repo_id,
                        filename=settings.clip_model_path,
                        verbose=settings.verbose,
                    )
                ...
                chat_handler = llamarcp.llama_chat_format.MoondreamChatHandler(
                    clip_model_path=settings.clip_model_path, verbose=settings.verbose
                )
            ...
            if settings.hf_model_repo_id is not None:
                chat_handler = (
                    llamarcp.llama_chat_format.NanoLlavaChatHandler.from_pretrained(
                        repo_id=settings.hf_model_repo_id,
                        filename=settings.clip_model_path,
                        verbose=settings.verbose,
                    )
                ...
                chat_handler = llamarcp.llama_chat_format.NanoLlavaChatHandler(
                    clip_model_path=settings.clip_model_path, verbose=settings.verbose
                )
            ...
            if settings.hf_model_repo_id is not None:
                chat_handler = (
                    llamarcp.llama_chat_format.Llama3VisionAlpha.from_pretrained(
                        repo_id=settings.hf_model_repo_id,
                        filename=settings.clip_model_path,
                        verbose=settings.verbose,
                    )
                ...
                chat_handler = llamarcp.llama_chat_format.Llama3VisionAlpha(
                    clip_model_path=settings.clip_model_path, verbose=settings.verbose
                )
            ...
            if settings.hf_model_repo_id is not None:
                chat_handler = (
                    llamarcp.llama_chat_format.MiniCPMv26ChatHandler.from_pretrained(
                        repo_id=settings.hf_model_repo_id,
                        filename=settings.clip_model_path,
                        verbose=settings.verbose,
                    )
                ...
                chat_handler = llamarcp.llama_chat_format.MiniCPMv26ChatHandler(
                    clip_model_path=settings.clip_model_path, verbose=settings.verbose
                )
            ...
            if settings.hf_model_repo_id is not None:
                chat_handler = (
                    llamarcp.llama_chat_format.Qwen25VLChatHandler.from_pretrained(
                        repo_id=settings.hf_model_repo_id,
                        filename=settings.clip_model_path,
                        verbose=settings.verbose,
                    )
                ...
                chat_handler = llamarcp.llama_chat_format.Qwen25VLChatHandler(
                    clip_model_path=settings.clip_model_path, verbose=settings.verbose
                )
            ...
            chat_handler = (
                llamarcp.llama_chat_format.hf_autotokenizer_to_chat_completion_handler(
            ...
            chat_handler = llamarcp.llama_chat_format.hf_tokenizer_config_to_chat_completion_handler(
                json.load(open(settings.hf_tokenizer_config_path))
            )

        tokenizer: Optional[llamarcp.BaseLlamaTokenizer] = None
        ...
        if settings.hf_model_repo_id is not None:
            create_fn = functools.partial(
                llamarcp.Llama.from_pretrained,
                repo_id=settings.hf_model_repo_id,
                filename=settings.model,
            )
        else:
            create_fn = llamarcp.Llama
        ...
        if settings.cache:
            if settings.cache_type == "disk":
                ...
                cache = llamarcp.LlamaDiskCache(capacity_bytes=settings.cache_size)
            else:
                if settings.verbose:
                    print(f"Using ram cache with size {settings.cache_size}")
                cache = llamarcp.LlamaRAMCache(capacity_bytes=settings.cache_size)
