...
class LlamaModel:
    ...
    def __init__(
        ...
        *,
        path_model: str,
        params: llamarcp.llama_model_params,
        verbose: bool = True,
    ):
        ...
        with suppress_stdout_stderr(disable=verbose):
            model = llamarcp.llama_model_load_from_file(
        ...
        vocab = llamarcp.llama_model_get_vocab(model)
        ...
        def free_model():
            if self.model is None:
                return
            llamarcp.llama_model_free(self.model)
            self.model = None
    ...
    def vocab_type(self) -> int:
        return llamarcp.llama_vocab_type(self.vocab)

    def n_vocab(self) -> int:
        return llamarcp.llama_vocab_n_tokens(self.vocab)

    def n_ctx_train(self) -> int:
        return llamarcp.llama_model_n_ctx_train(self.model)

    def n_embd(self) -> int:
        return llamarcp.llama_model_n_embd(self.model)

    def rope_freq_scale_train(self) -> float:
        return llamarcp.llama_model_rope_freq_scale_train(self.model)

    def desc(self) -> str:
        buf = ctypes.create_string_buffer(1024)
        llamarcp.llama_model_desc(self.model, buf, 1024)
        return buf.value.decode("utf-8")

    def size(self) -> int:
        return llamarcp.llama_model_size(self.model)

    def n_params(self) -> int:
        return llamarcp.llama_model_n_params(self.model)
    ...
    def token_get_text(self, token: int) -> str:
        return llamarcp.llama_vocab_get_text(self.vocab, token).decode("utf-8")

    def token_get_score(self, token: int) -> float:
        return llamarcp.llama_vocab_get_score(self.vocab, token)

    def token_get_attr(self, token: int) -> int:
        return llamarcp.llama_vocab_get_attr(self.vocab, token)
    ...
    def add_eos_token(self) -> bool:
        return llamarcp.llama_vocab_get_add_eos(self.vocab)

    # Tokenization

    def tokenize(self, text: bytes, add_bos: bool, special: bool):
        n_ctx = self.n_ctx_train()
        tokens = (llamarcp.llama_token * n_ctx)()
        n_tokens = llamarcp.llama_tokenize(
            self.vocab, text, len(text), tokens, n_ctx, add_bos, special
        )
        if n_tokens < 0:
            n_tokens = abs(n_tokens)
            tokens = (llamarcp.llama_token * n_tokens)()
            n_tokens = llamarcp.llama_tokenize(
    ...
    def token_to_piece(self, token: int, special: bool = False) -> bytes:
        ...
        llamarcp.llama_token_to_piece(self.vocab, token, buf, 32, 0, special)
        return bytes(buf)
    ...
    def detokenize(self, tokens: List[int], special: bool = False) -> bytes:
        ...
        for token in tokens:
            n = llamarcp.llama_token_to_piece(
                self.vocab, llamarcp.llama_token(token), buffer, size, 0, special
            )
            assert n <= size
    ...
    def metadata(self) -> Dict[str, str]:
        ...
        for i in range(llamarcp.llama_model_meta_count(self.model)):
            nbytes = llamarcp.llama_model_meta_key_by_index(
            ...
            nbytes = llamarcp.llama_model_meta_val_str_by_index(
                self.model, i, buffer, buffer_size
            )
            if nbytes > buffer_size:
                buffer_size = nbytes + 1
    ...
    def default_params():
        ...
        return llamarcp.llama_model_default_params()
...
class LlamaContext:
    ...
    def __init__(
        ...
        *,
        model: LlamaModel,
        params: llamarcp.llama_context_params,
        verbose: bool = True,
    ):
        self.model = model
        self.params = params
        ...
        ctx = llamarcp.llama_init_from_model(self.model.model, self.params)

        if ctx is None:
            raise ValueError("Failed to create llama_context")

        self.ctx = ctx
        self.memory = llamarcp.llama_get_memory(self.ctx)
        self.sampler = None  # LlamaContext doesn't manage samplers directly, but some cleanup code expects this attribute

        def free_ctx():
            if self.ctx is None:
                return
            llamarcp.llama_free(self.ctx)
            self.ctx = None
    ...
    def n_ctx(self) -> int:
        return llamarcp.llama_n_ctx(self.ctx)

    def pooling_type(self) -> int:
        return llamarcp.llama_pooling_type(self.ctx)

    def kv_cache_clear(self):
        assert self.memory is not None, "Memory is not initialized"
        llamarcp.llama_memory_clear(self.memory, True)

    def kv_cache_seq_rm(self, seq_id: int, p0: int, p1: int):
        assert self.memory is not None, "Memory is not initialized"
        seq_id = seq_id if seq_id >= 0 else 0
        llamarcp.llama_memory_seq_rm(self.memory, seq_id, p0, p1)

    def kv_cache_seq_cp(self, seq_id_src: int, seq_id_dst: int, p0: int, p1: int):
        assert self.memory is not None, "Memory is not initialized"
        llamarcp.llama_memory_seq_cp(self.memory, seq_id_src, seq_id_dst, p0, p1)

    def kv_cache_seq_keep(self, seq_id: int):
        assert self.memory is not None, "Memory is not initialized"
        llamarcp.llama_memory_seq_keep(self.memory, seq_id)

    def kv_cache_seq_shift(self, seq_id: int, p0: int, p1: int, shift: int):
        assert self.memory is not None, "Memory is not initialized"
        llamarcp.llama_memory_seq_add(self.memory, seq_id, p0, p1, shift)

    def get_state_size(self) -> int:
        return llamarcp.llama_state_get_size(self.ctx)
    ...
    def decode(self, batch: LlamaBatch):
        return_code = llamarcp.llama_decode(
            self.ctx,
            batch.batch,
        )
        if return_code != 0:
    ...
    def encode(self, batch: LlamaBatch):
        return_code = llamarcp.llama_encode(
            self.ctx,
            batch.batch,
        )
        if return_code != 0:
    ...
    def set_n_threads(self, n_threads: int, n_threads_batch: int):
        llamarcp.llama_set_n_threads(self.ctx, n_threads, n_threads_batch)

    def get_logits(self):
        return llamarcp.llama_get_logits(self.ctx)

    def get_logits_ith(self, i: int):
        return llamarcp.llama_get_logits_ith(self.ctx, i)

    def get_embeddings(self):
        return llamarcp.llama_get_embeddings(self.ctx)

    def get_embeddings_ith(self, i: int):
        return llamarcp.llama_get_embeddings_ith(self.ctx, i)

    def get_embeddings_seq(self, seq_id: int):
        return llamarcp.llama_get_embeddings_seq(self.ctx, seq_id)
    ...
    def print_timings(self):
        llamarcp.llama_perf_context_print(self.ctx)

    # Utility functions
    @staticmethod
    def default_params():
        """Get the default llama_context_params."""
        return llamarcp.llama_context_default_params()
...
class LlamaBatch:
    def __init__(
        ...
        batch = llamarcp.llama_batch_init(self._n_tokens, self.embd, self.n_seq_max)
        ...
        def free_batch():
            if self.batch is None:
                return
            llamarcp.llama_batch_free(self.batch)
            self.batch = None
...
class LlamaTokenDataArray:
    def __init__(self, *, n_vocab: int):
        ...
        self.candidates = llamarcp.llama_token_data_array(
            data=self.candidates_data.ctypes.data_as(llamarcp.llama_token_data_p),
            size=self.n_vocab,
            sorted=False,
        )
...
class LlamaSamplingContext:
    ...
    cur: list[llamarcp.llama_token_data] = field(default_factory=list)
...
class CustomSampler:
    def __init__(
        ...
        def free_wrapper(sampler: llamarcp.llama_sampler_p):
            pass

        sampler_i = llamarcp.llama_sampler_i()
        sampler_i.apply = llamarcp.llama_sampler_i_apply(apply_wrapper)
        self._apply_wrapper_ref = apply_wrapper

        sampler_i.name = llamarcp.llama_sampler_i_name(0)
        sampler_i.accept = llamarcp.llama_sampler_i_accept(0)
        sampler_i.reset = llamarcp.llama_sampler_i_reset(0)
        sampler_i.clone = llamarcp.llama_sampler_i_clone(0)
        sampler_i.free = llamarcp.llama_sampler_i_free(0)

        self.sampler = llamarcp.llama_sampler()
        self.sampler.iface = ctypes.pointer(sampler_i)
        self.sampler.ctx = None

    def get_sampler(self) -> llamarcp.llama_sampler_p:
        return ctypes.pointer(self.sampler)


class LlamaSampler:
    def __init__(self):
        params = llamarcp.llama_sampler_chain_default_params()
        self.sampler = llamarcp.llama_sampler_chain_init(params)
        ...
        def free_sampler():
            if self.sampler is not None:
                ...
                for i, _ in reversed(self.custom_samplers):
                    llamarcp.llama_sampler_chain_remove(self.sampler, i)
                llamarcp.llama_sampler_free(self.sampler)
                self.sampler = None
    ...
    def add_greedy(self):
        sampler = llamarcp.llama_sampler_init_greedy()
        llamarcp.llama_sampler_chain_add(self.sampler, sampler)

    def add_dist(self, seed: int):
        sampler = llamarcp.llama_sampler_init_dist(seed)
        llamarcp.llama_sampler_chain_add(self.sampler, sampler)
    ...
    def add_top_p(self, p: float, min_keep: int = 1):
        sampler = llamarcp.llama_sampler_init_top_p(p, min_keep)
        llamarcp.llama_sampler_chain_add(self.sampler, sampler)

    def add_min_p(self, p: float, min_keep: int = 1):
        sampler = llamarcp.llama_sampler_init_min_p(p, min_keep)
        llamarcp.llama_sampler_chain_add(self.sampler, sampler)

    def add_typical(self, p: float, min_keep: int = 1):
        sampler = llamarcp.llama_sampler_init_typical(p, min_keep)
        llamarcp.llama_sampler_chain_add(self.sampler, sampler)

    def add_temp(self, temp: float):
        sampler = llamarcp.llama_sampler_init_temp(temp)
        llamarcp.llama_sampler_chain_add(self.sampler, sampler)

    def add_temp_ext(self, t: float, delta: float, exponent: float):
        sampler = llamarcp.llama_sampler_init_temp_ext(t, delta, exponent)
        llamarcp.llama_sampler_chain_add(self.sampler, sampler)

    def add_xtc(self, p: float, t: float, min_keep: int, seed: int):
        sampler = llamarcp.llama_sampler_init_xtc(p, t, min_keep, seed)
        llamarcp.llama_sampler_chain_add(self.sampler, sampler)

    def add_top_n_sigma(self, n: float):
        sampler = llamarcp.llama_sampler_init_top_n_sigma(n)
        llamarcp.llama_sampler_chain_add(self.sampler, sampler)

    def add_mirostat(self, n_vocab: int, seed: int, tau: float, eta: float, m: int):
        sampler = llamarcp.llama_sampler_init_mirostat(n_vocab, seed, tau, eta, m)
        llamarcp.llama_sampler_chain_add(self.sampler, sampler)

    def add_mirostat_v2(self, seed: int, tau: float, eta: float):
        sampler = llamarcp.llama_sampler_init_mirostat_v2(seed, tau, eta)
        llamarcp.llama_sampler_chain_add(self.sampler, sampler)

    def add_grammar(self, model: LlamaModel, grammar: LlamaGrammar):
        sampler = llamarcp.llama_sampler_init_grammar(
    ...
    def add_penalties(
        ...
        sampler = llamarcp.llama_sampler_init_penalties(
            penalty_last_n,
            penalty_repeat,
            penalty_freq,
            penalty_present,
    ...
    def add_custom(
        ...
        self.custom_samplers.append(
            (llamarcp.llama_sampler_chain_n(self.sampler) - 1, custom_sampler)
        )

    def get_seed(self) -> int:
        return llamarcp.llama_sampler_get_seed(self.sampler)

    def sample(self, ctx: LlamaContext, idx: int = -1) -> int:
        return llamarcp.llama_sampler_sample(self.sampler, ctx.ctx, idx)

    def accept(self, token: int):
        llamarcp.llama_sampler_accept(self.sampler, token)

    def reset(self):
        llamarcp.llama_sampler_reset(self.sampler)
