...
from llamarcp.server.model import (
    LlamaProxy,
)
from llamarcp.server.settings import (
    ConfigFileSettings,
    Settings,
    ModelSettings,
    ServerSettings,
)
from llamarcp.server.types import (
...
from llamarcp.server.errors import RouteErrorHandler
...
def prepare_request_resources(
    ...
    if body.grammar is not None:
        kwargs["grammar"] = llamarcp.LlamaGrammar.from_string(body.grammar)

    if body.min_tokens > 0:
        _min_tokens_logits_processor = llamarcp.LogitsProcessorList(
            [llamarcp.MinTokensLogitsProcessor(body.min_tokens, llama.token_eos())]
        )
...
@router.post(
    ...
    response_model=Union[
        llamarcp.CreateCompletionResponse,
        str,
    ],
    responses={
        "200": {
    ...
    if kwargs.get("stream", False):
        ...
        return EventSourceResponse(
            ...
            data_sender_callable=partial(  # type: ignore
                ...
                llama_call=llamarcp.Llama.__call__,
                kwargs=kwargs,
            ),
            sep="\n",
...
@router.post(
    ...
    summary="Chat",
    dependencies=[Depends(authenticate)],
    response_model=Union[llamarcp.ChatCompletion, str],
    responses={
        "200": {
            "description": "Successful Response",
            "content": {
    ...
    if kwargs.get("stream", False):
        ...
        return EventSourceResponse(
            ...
            data_sender_callable=partial(  # type: ignore
                ...
                body=body,
                body_model=body_model,
                llama_call=llamarcp.Llama.create_chat_completion,
                kwargs=kwargs,
            ),
            sep="\n",
            ping_message_factory=_ping_message_factory,
