...
import llamarcp._internals as internals
...
class Llama:
    ...
    def __init__(
        ...
        *,
        # Model Params
        n_gpu_layers: int = 0,
        split_mode: int = llamarcp.LLAMA_SPLIT_MODE_LAYER,
        main_gpu: int = 0,
        ...
        # Context Params
        seed: int = llamarcp.LLAMA_DEFAULT_SEED,
        n_ctx: int = 512,
        n_batch: int = 512,
        n_ubatch: int = 512,
        ...
        **kwargs,  # type: ignore
        ...
        Args:
            ...
            split_mode: How to split the model across GPUs. See llamarcp.LLAMA_SPLIT_* for options.
        ...
        if not Llama.__backend_initialized:
            with suppress_stdout_stderr(disable=verbose):
                llamarcp.llama_backend_init()
            Llama.__backend_initialized = True

        if isinstance(numa, bool):
            self.numa = (
                llamarcp.GGML_NUMA_STRATEGY_DISTRIBUTE
                if numa
                else llamarcp.GGML_NUMA_STRATEGY_DISABLED
            )
        else:
            self.numa = numa

        if self.numa != llamarcp.GGML_NUMA_STRATEGY_DISABLED:
            with suppress_stdout_stderr(disable=verbose):
                llamarcp.llama_numa_init(self.numa)

        self.model_path = model_path

        # Model Params
        self.model_params = llamarcp.llama_model_default_params()
        ...
        if self.tensor_split is not None:
            if len(self.tensor_split) > llamarcp.LLAMA_MAX_DEVICES:
                raise ValueError(
                    f"Attempt to split tensors that exceed maximum supported devices. Current LLAMA_MAX_DEVICES={llamarcp.LLAMA_MAX_DEVICES}"
                )
            # Type conversion and expand the list to the length of LLAMA_MAX_DEVICES
            FloatArray = ctypes.c_float * llamarcp.LLAMA_MAX_DEVICES
        ...
        if kv_overrides is not None:
            ...
            self._kv_overrides_array = (
                llamarcp.llama_model_kv_override * kvo_array_len
            )()
            ...
            for i, (k, v) in enumerate(kv_overrides.items()):
                ...
                if isinstance(v, bool):
                    ...
                    address = typing.cast(
                        ...
                        + llamarcp.llama_model_kv_override_value.val_str.offset,
                    )
        ...
        # Used by the sampler
        self._seed = seed or llamarcp.LLAMA_DEFAULT_SEED

        # Context Params
        self.context_params = llamarcp.llama_context_default_params()
        ...
        self.context_params.rope_scaling_type = (
            ...
            else llamarcp.LLAMA_ROPE_SCALING_TYPE_UNSPECIFIED
        )
        ...
        self._lora_adapter: Optional[llamarcp.llama_adapter_lora_p] = None

        if self.lora_path:
            self._lora_adapter = llamarcp.llama_adapter_lora_init(
                self._model.model,
            ...
            def free_lora_adapter():
                if self._lora_adapter is None:
                    return
                llamarcp.llama_adapter_lora_free(self._lora_adapter)
                self._lora_adapter = None

            self._stack.callback(free_lora_adapter)

            if llamarcp.llama_set_adapter_lora(
        ...
        if self.verbose:
            print(llamarcp.llama_print_system_info().decode("utf-8"), file=sys.stderr)
    ...
    @property
    def ctx(self) -> llamarcp.llama_context_p:
        return self._ctx.ctx

    @property
    def model(self) -> llamarcp.llama_model_p:
    ...
    def _init_sampler(
        ...
        if logits_processor is not None:
            ...
            def apply_func(token_data_array: llamarcp.llama_token_data_array_p):
                ...
                recarray = np.recarray(
                    ...
                    buf=(llamarcp.llama_token_data * size).from_address(
                        data_soa_address
                    ),
                )
    ...
    def embed(
        ...
        logits_all = pooling_type == llamarcp.LLAMA_POOLING_TYPE_NONE
        ...
        if self.verbose:
            llamarcp.llama_perf_context_reset(self._ctx.ctx)
        ...
        def decode_batch(seq_sizes: List[int]):
            ...
            # store embeddings
            if pooling_type == llamarcp.LLAMA_POOLING_TYPE_NONE:
                pos: int = 0
                for i, size in enumerate(seq_sizes):
                    ptr = llamarcp.llama_get_embeddings(self._ctx.ctx)
                ...
                for i in range(len(seq_sizes)):
                    ptr = llamarcp.llama_get_embeddings_seq(self._ctx.ctx, i)
        ...
        if self.verbose:
            llamarcp.llama_perf_context_print(self._ctx.ctx)
    ...
    def _create_completion(
        ...
        if len(prompt_tokens) >= self._n_ctx:
            raise ValueError(
                f"Requested tokens ({len(prompt_tokens)}) exceed context window of {llamarcp.llama_n_ctx(self.ctx)}"
            )
        ...
        for token in self.generate(
            ...
            if llamarcp.llama_token_is_eog(self._model.vocab, token):
    ...
    def save_state(self) -> LlamaState:
        ...
        state_size = llamarcp.llama_get_state_size(self._ctx.ctx)
        if self.verbose:
            print(f"Llama.save_state: got state size: {state_size}", file=sys.stderr)
        llama_state = (ctypes.c_uint8 * int(state_size))()
        if self.verbose:
            print("Llama.save_state: allocated state", file=sys.stderr)
        n_bytes = llamarcp.llama_copy_state_data(self._ctx.ctx, llama_state)
        if self.verbose:
            print(f"Llama.save_state: copied llama state: {n_bytes}", file=sys.stderr)
        if int(n_bytes) > int(state_size):
            raise RuntimeError("Failed to copy llama state data")
        llama_state_compact = (ctypes.c_uint8 * int(n_bytes))()
        llamarcp.ctypes.memmove(llama_state_compact, llama_state, int(n_bytes))
        if self.verbose:
            print(
    ...
    def load_state(self, state: LlamaState) -> None:
        ...
        if llamarcp.llama_set_state_data(self._ctx.ctx, llama_state) != state_size:
